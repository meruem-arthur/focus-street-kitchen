...
function MenuManagementPage() {
  ...
  async function savePrice(id: number, categoryId: number, name: string, price: number) {
    await saveMenuItem({ data: { id, categoryId, name, price, available: true } });
    queryClient.invalidateQueries({ queryKey: ["admin-menu"] });
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-xs text-ink/45">
        Toggle availability instantly, or tap a price to edit it. Adding brand-new categories/items isn't
                  ...
                  <input
                    ...
                    onBlur={(e) => {
                      ...
                      if (!Number.isNaN(val) && val > 0 && val !== item.price) {
                        savePrice(item.id, cat.id, item.name, val);
                      }
                    }}
